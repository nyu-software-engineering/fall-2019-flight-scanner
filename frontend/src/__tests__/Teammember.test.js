...
// const randomNumber = jest.fn(()=>1258); 

it('renders without crashing', () => {
  const div = document.createElement('div');
  ReactDOM.render(<Teammember />, div);
  ReactDOM.unmountComponentAtNode(div);
});

it('getFirstName test', () =>{
    expect(getFirstName("Anonymous User")).toEqual("Anonymous"); 
    expect(getFirstName("AnonymousUser")).toEqual("Invalid Name"); 
}); 

it('validateRole test', () =>{
    expect(validateRole("Admin")).toEqual("Admin"); 
    expect(validateRole("Classified Rol")).toEqual("Invalid Role"); 
}); 

it('trimBio test', () =>{
    expect(trimBio("My Bio")).toBeLessThanOrEqual(100);
    expect(trimBio(testString)).toBeLessThanOrEqual(100);  
}); 

it('checkAuthority test', () =>{
    expect(checkAuthority("Admin", "Add Editor")).toBeTruthy;
    expect(checkAuthority("Editor", "Delete Editor")).toBeFaulty;  
}); 
...
it('generateUserID test', () =>{
    expect(generateUserID("abloomberg")).toHaveLength(6); 
    expect(generateUserID("a")).toEqual("******"); 
}); 

it('filterBadWords test', ()=>{
    expect(filterBadWords('fuck')).not.toMatch(/fuck/); 
    expect(filterBadWords('ffff')).not.toMatch(/fuck/); 
}); 

it('generateInfoArray test', ()=>{
    expect(generateInfoArray("Anonymous User", "Admin")).toContain('Admin'); 
    expect(generateInfoArray("Anonymous User", "Muahaha")).toContain('Invalid'); 
}); 

it('testMakeRolesBinary test', () =>{
